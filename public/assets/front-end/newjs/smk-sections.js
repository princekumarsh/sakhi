...
Line 1332:8 Transforming const to the configured target environment ("es5") is not supported yet
...
! function(i) {
    ...
    (e = function() {
        ...
        null !== t && "object" == typeof t && t.each(function() {
            var t = i(this).smkSlick("getSlick");
        ...
        if (e = e || !1, !(!0 === a.animating && !0 === a.options.waitForAnimate || !0 === a.options.fade && a.currentSlide === i))
            ...
            if (a.options.autoplay && clearInterval(a.autoPlayTimer), s = o < 0 ? a.slideCount % a.options.slidesToScroll != 0 ? a.slideCount - a.slideCount % a.options.slidesToScroll : a.slideCount + o : o >= a.slideCount ? a.slideCount % a.options.slidesToScroll != 0 ? 0 : o - a.slideCount : o, a.animating = !0, a.$slider.trigger("beforeChange", [a, a.currentSlide, s]), n = a.currentSlide, a.currentSlide = s, a.setSlideClasses(a.currentSlide), a.options.asNavFor && (d = (d = a.getNavTarget()).smkSlick("getSlick")).slideCount <= d.options.slidesToShow && d.setSlideClasses(a.currentSlide), a.updateDots(), a.updateArrows(), !0 === a.options.fade) return !0 !== t ? (a.fadeSlideOut(n), a.fadeSlide(s, function() {
                a.postSlide(s)
    ...
    }, i.fn.smkSlick = function() {
        var i, t, o = this,
            s = arguments[0],
            n = Array.prototype.slice.call(arguments, 1),
            r = o.length;
        for (i = 0; i < r; i++)
            if ("object" == typeof s || void 0 === s ? o[i].smkSlick = new e(o[i], s) : t = o[i].smkSlick[s].apply(o[i].smkSlick, n), void 0 !== t) return t;
        return o
    }
});
...
;
(function() {
    $.fn.smk_text_plus_image = function() {
        $('[data-tpi-video-link]').click(function(e) {
            var video = $(e.target).attr('data-tpi-video-link');
            if (!$('[data-tpi-video="' + video + '"]').length) {
                $('body').prepend('<div data-tpi-video="' + video + '"></div>');
            }
            $('[data-tpi-video="' + video + '"]').html('<video controls autoplay="autoplay"><source src="' + video + '"></video>' +
                '<a class="tpi-video-close"><i class="fas fa-times"></i></a>').show();
            $('[data-tpi-video="' + video + '"]').click(function() {
                $(this).hide();
            });
            //$('[data-tpi-video] video').play();
        });
    };
    ...
    $.fn.smkInitSingleTextWithIcon = function() {
        ...
        function unslickSingleTextWithIcon() {
            ...
            if (!$singleTextWithIcons.hasClass('slick-slider')) return false;
            setTimeout(function() {
                $singleTextWithIcons.smkSlick('unslick');
            }, 200);
    ...
    $.fn.smk_video_banner = function() {
        setTimeout(function() {
            $('.smk-video video').each(function() {
                if (($(window).width() > 970 && $(this).attr('mobile-src')) || !$(this).attr('mobile-src')) {
    ...
    $.fn.smk_product_offers = function() {
        ...
        $('.smk-product-offer-btn').on('click', function(event) {
            ...
            var plan = $(target).attr('data-selling-plan');
            var timeout = 0;
            if (behaviour) {
                        ...
                        switch (behaviour) {
                            ...
                            case 'add_to_cart':
                                ...
                                if (quantity && variant) {
                                    ...
                                    if (plan) cart['selling-plan'] = plan;
                                    $.post('/cart/add.js', {
                                        items: [cart]
                                    }).always(function() {
                                        console.log('added');
    ...
    $.fn.smk_cta_button = function() {
        $('.smk-cta-btn').on('click', function(event) {
            ...
            var timeout = 0;
            if (behaviour && !Shopify.designMode) {
                setTimeout(function() {
                    switch (behaviour) {
                        case 'product_page':
                            console.log('CLICK: Product page');
                        ...
                        case 'video_popup':
                            if (url) {
                                if (!$('[data-tpi-video="' + url + '"]').length) {
                                    $('body').prepend('<div data-tpi-video="' + url + '"></div>');
                                }
                                $('[data-tpi-video="' + url + '"]').html('<video controls autoplay="autoplay"><source src="' + url + '"></video>' +
                                    '<a class="tpi-video-close"><i class="fas fa-times"></i></a>').show();
                                $('[data-tpi-video="' + url + '"]').click(function() {
                                    $(this).hide();
                                });
                            }
                            break;
    ...
    $.fn.smk_smart_products = function() {
        ...
        request.onload = function() {
            ...
            localStorage.setItem('smkSmartProducts', JSON.stringify(products));
            ...
            window.addSmartToCart = addSmartToCart;
            window.hideQuickView = hideQuickView;
            attachQuickView();
        };
        ...
        function renderProduct(product) {
            var config = document.querySelector(".product-recommendations__list");
            var quickViewType = config.dataset.smkPbQvT;

            var productItem = '<div class="smk-smart-product-item">';
            productItem += '<div class="smk-add-to-cart-icon" onclick="addSmartToCart(' + product.id + ')"><i class="fas fa-cart-plus" aria-hidden="true"></i></div>';
            productItem += '<a href="' + product.url + '" class="product__anchor">' +
                '<img class="product__img" src="' + product.featured_image + '" alt="' + product.title + '" style="max-width: 100%;">' +
                '<p class="product__title">' + product.title + '</p>' +
                '<p class="product__price">' + formatMoney(product.price, window.moneyFormat) + '<span class="old-price">' + formatMoneyP(product.price, window.moneyFormat) + '</span></p>';
            productItem += '</a>';
            if (quickViewType != 'qv_btn_hide') {
                productItem += '<div class="smk-smart-product-quick-btn" onclick="quickView(' + product.id + ', ' + product.title + ')">QUICK VIEW</div>';
            }
            productItem += '<div class="smk-smart-product-btn" onclick="addSmartToCart(' + product.id + ')">ADD TO CART</div>';
            productItem += '</div>';
            return productItem;
        }
        ...
        function quickView(productId, productName) {
            ...
            /*var quickView = '<div class="smk-quick-view-wrap">' +
                '<div class="smk-quick-view">' +
                '</div>' +
                '</div>';
            if ($('.smk-quick-view-wrap').length) {
                $('.smk-quick-view-wrap').remove();
            }
            $('body').css('overflow', 'hidden').prepend(quickView);*/

            var products = JSON.parse(localStorage.getItem('smkSmartProducts'));
            /*if (parent.smarketly.id != undefined && parent.smarketly.id != 1) {
                $.get('https://apps.smarketly.com/stats/' + parent.smarketly.id + ' /track/click/product/' + productId + '/name/' + productName, function(data) {
                });    
            }*/
        ...
        function hideQuickView() {
            ...
            $('.quick-view-wrap, .smk-quick-view-wrap').hide();
        }

        function addSmartToCart(productId) {
            var products = JSON.parse(localStorage.getItem('smkSmartProducts'));
        ...
        function formatMoneyP(price, format) {
            return '$' + parseFloat((price + 1000) / 100).toFixed(2);
        }

    };

    $.fn.smk_upsells_cross_sells = function() {
    ...
    $.fn.smkInitSection = function() {
        ...
        switch (n) {
            case ("smk_text_plus_image"):
                _.smk_text_plus_image();
                break;
            case ("smk_big_banners"):
                _.smk_big_banners();
                break;
            case ("smk_instagram"):
                _.smk_instagram();
                break;
            case ("smk_brands_slider"):
                _.find('.smk-carousel-brands').smkInitSlickBrands(6, 4, 3, 2, 1);
                break;
            case ("smk_testimonials"):
                _.find('.smk-slider-fullwidth').smkInitSlick(1, 1, 1);
                break;
            case ("smk_faq"):
                _.smk_faq();
                break;
            case ("smk_revolution_slider"):
                _.find('.smk-revolution-slider').smkInitRevolution();
                break;
            case ("smk_icon_features_bar"):
                _.find('.smk-single-text-with-icons').smkInitSingleTextWithIcon();
                break;
            case ("smk_icon_banners"):
                _.find('.smk-single-icon-banner').smkInitSingleTextWithIcon();
                break;
            case ("smk_video_banner"):
                _.smk_video_banner();
                break;
            case ("smk_product_offers"):
                _.smk_product_offers();
                break;
            case ("smk_cta_button"):
                _.smk_cta_button();
                break;
            case ("smk_dynamic_product"):
                _.smk_dynamic_product();
                break;
            case ("smk_smart_products"):
                _.smk_smart_products();
                break;
            case ("smk_upsells_cross_sells"):
                _.smk_upsells_cross_sells();
                break;
            default:
                break;
        }
    ...
    $(document)
        ...
        .ready(function() {
            if ((!Shopify.designMode && Shopify.oseid) && (parent.location.href.includes('/admin/themes') && $('[component="PageSelector"] span[bind="currentPageTitle"]', parent.document).text() != 'Home page' && !parent.location.hash.includes('#/pages') && $('.smk-sections-above-content').length && $('.smk-sections-below-content').length && $('.smk-sections-above-footer').length) ||
            ...
            $('[data-smk-section-type]').each(function() {
                $(this).smkInitSection()
            });
    ...
    function eventHandler(e) {
        ...
        $('#' + e.target.id).find('[data-smk-section-type]').each(function() {
            $(this).smkInitSection()
        });
        addSectionsPlaceholders(0);
    }

    function installSectionsOnPage(pageId) {
        ...
        var updateMessage = loaderStyles + '<div class="smk-version-message" style="margin-left: 0 auto; display:flex; flex-direction: column; align-items: center;">' +
            '<div style="color: #404040; margin-top: 20px;">Installing sections on the page...</div>' +
            '<div class="smk-update-loader" style="margin: 20px auto;"></div>' +
            '</div>';
        $('.smk-sections-above-content .smk-section-placeholder .smk-install-sections-btn-wrap').html(updateMessage);

        if (Shopify.shop) {
            $.post('https://apps.smarketly.com/app/productBlocks/action/installSectionsOnPage', {
                shop: Shopify.shop,
                pageId: pageId
            }).always(function() {
                $('.smk-sections-above-content .smk-section-placeholder .smk-install-sections-btn-wrap').html('<div style="color: #404040; margin-top: 20px;">Sections installed successfully.<br>Shopify editor will now reload.</div>');
                setTimeout(function() {
    ...
    window.installSectionsOnPage = installSectionsOnPage;
    ...
    function addSectionsPlaceholders(timeout) {
        ...
        setTimeout(function() {
            areas.forEach(function(area) {
                var placeholder = '<div class="smk-section-placeholder" style="position: relative;">' +
                    ...
                    '<span class="smk-desktop-new-line"><br></span><span class="smk-position-line">To add it here, choose "<b>' + area.type + '</b>" in Section location dropdown in your newly added section\'s settings.</span>' +
                    '</div>' +
                    '<div class="smk-section-placeholder-message" style="">This placeholder does not appear on the published page</div>' +
                    '</div>';

                if (Shopify.designMode && Shopify.oseid) {

                    if ($('.' + area.class).length &&
                        $('.' + area.class).text() == '' &&
                    ...
                    if ($('#shopify-section-smk_blocks').length &&
            ...
            /* Detect if page contains section and insert button to install them */

            var pageId = null;
            var smkSectionFound = false;
            ...
            if (pageId) {
                $('[data-shopify-editor-section]').each(function() {
                    try {
                        var sectionConfig = JSON.parse($(this).attr('data-shopify-editor-section'));
                        if (sectionConfig.type == 'smk_blocks_' + pageId) {
                            smkSectionFound = true;
                        }
                    } catch (e) {}
                });
            }

            if (Shopify.designMode && pageId && !smkSectionFound) {
                //$('.' + areas[0].class + ' .smk-section-placeholder').html('<img src="https://apps.shopifycdn.com/listing_images/fc97048a562c9737f3b4b122bd889d2e/icon/3c95f528a90d160701637b2c851992d9.png?height=84&width=84" style="border-radius: 8px; margin-top: 10px;"><div style="text-align: center; color: #404040; font-size: 22px;">Smarketly Page Builder Editor</div>'+
                //                                                         '<div class="smk-install-sections-btn-wrap"><div class="smk-install-sections-btn" onclick="installSectionsOnPage(' + pageId + ')">Install Smarketly Sections on this page</div></div>');
                $('.' + areas[0].class + ' .smk-section-placeholder .smk-section-placeholder-description').html('<div class="smk-install-sections-btn-wrap"><div class="smk-install-sections-btn" onclick="installSectionsOnPage(' + pageId + ')">Install Smarketly Sections on this page</div></div>');
                $('.' + areas[0].class + ' .smk-section-placeholder .smk-position-line').hide();
                areas.forEach(function(area) {
                    if (area.class != 'smk-sections-above-content') {
                        $('.' + area.class + ' .smk-section-placeholder').hide();
                    }
                });
            }
        }, timeout);
