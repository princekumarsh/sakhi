...
Line 1384:26 Transforming destructuring to the configured target environment ("es5") is not supported yet
Line 1384:57 Transforming default arguments to the configured target environment ("es5") is not supported yet
Line 1392:63 Transforming default arguments to the configured target environment ("es5") is not supported yet
Line 1393:8 Transforming const to the configured target environment ("es5") is not supported yet
Line 1394:8 Transforming const to the configured target environment ("es5") is not supported yet
Line 1396:8 Transforming const to the configured target environment ("es5") is not supported yet
Line 1398:10 Transforming const to the configured target environment ("es5") is not supported yet
Line 1399:10 Transforming const to the configured target environment ("es5") is not supported yet
Line 1412:8 Transforming const to the configured target environment ("es5") is not supported yet
Line 1426:14 Transforming const to the configured target environment ("es5") is not supported yet
    ...
    theme.Collection = (function () {
        ...
        Collection.prototype = Object.assign({}, Collection.prototype, {
            ...
            onFormSubmit: function (evt) {
                var el = evt.target;
                console.log('formlciked');
                /* '   $(".flickity-index-slider").flickity({
                       cellAlign: "left",
            ...
            initProductSlider: function (variant) {
                ...
                if (this.cache.mainSlider.querySelectorAll(selectors.slide).length <= 1) {
                    ...
                    if (slide) {
                        ...
                        slide.classList.add('is-selecteddot');
                    }
                    return;
                }
...
const elems = document.querySelectorAll(".select-choice");


for (var i = 0; i < elems.length; i++) {
    elems[i].addEventListener('click', function (event) {
        document.querySelector('.product-name').innerHTML = 'ADD TO CART';
        document.getElementById('div').classList.remove('dis');
    });
}
